...
const CreateManual: React.FC<ContainerProps> = ({ name }) => {
  ...
  const handleAddManual = async () => {
    try {
      ...
      if (manualImage) {
        const storageRef = ref(storage, `manual-images/${manualImage.name}`);
        await uploadBytes(storageRef, manualImage);
        const manualImageUrl = await getDownloadURL(storageRef);
